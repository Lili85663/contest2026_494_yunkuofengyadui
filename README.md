...
| docs/ | [运行教程](quickapp/hello_quickapp/docs/运行教程.md)、[文件功能说明](quickapp/hello_quickapp/docs/文件功能说明.md) |
...
完整首次安装步骤见[运行教程](quickapp/hello_quickapp/docs/运行教程.md)。已配置的 Ubuntu 电脑：
...
本地交付包中的 `logs/Lili85663/` 是本次 Codex 会话的**用户可见消息脱敏导出**。它不是组委会采集器原始日志，不含隐藏推理、凭据、原始工具输出及图片附件。条数、范围、脱敏方式见 export-manifest.json。不能据此声称满足不可修改原始日志要求；正式提交按[官方日志指南](https://github.com/open-vela/docs/blob/dev-ai-contest-2026/zh-cn/contest_2026/ai_coding_log_guide.md)核验，敏感原始会话不要直接提交。
...
本次提交准备目录：应用工程位于 `quickapp/hello_quickapp/`。对话日志尚未复制到公开提交目录；需核对正式日志要求与公开内容。原仓库 manifest、.github 等配置应保留。本目录用于合入已有 fork，不是创建无历史的新仓库。
